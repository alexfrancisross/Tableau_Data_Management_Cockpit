...
DATABASE_QUERY = 'query PagedFieldsQuery($afterToken: String) {databasesConnection(first:1000, after: $afterToken){ ' \
                 ...
                 'downstreamDatasources{ id, name, projectName, isCertified, __typename}, ' \
                 'downstreamMetrics{ id, name, projectName},' \
...
TABLE_QUERY = 'query PagedFieldsQuery($afterToken: String) {tablesConnection(first:1000, after: $afterToken) ' \
              ...
              'database { id}, hasActiveWarning, }, ' \
              '... on CustomSQLTable{ query, isUnsupportedCustomSql, tables{id}, database{ id}}, ' \
...
COLUMN_QUERY = 'query PagedFieldsQuery($afterToken: String) {columnsConnection(first:1000, after: $afterToken){ nodes' \
               ...
               'table { id}}, pageInfo {endCursor, hasNextPage }, totalCount} } '
FIELD_QUERY = "query PagedFieldsQuery($afterToken: String) {fieldsConnection(first:1000, after: $afterToken){ nodes {" \
              ...
              "__typename, datasource{ id}, downstreamWorkbooks{ id, name}, ... on BinField{ binSize, dataType, " \
              "dataCategory, role, formula, fields{ id}}, ... on CalculatedField{ dataCategory, role, dataType, " \
              "defaultFormat, semanticRole, aggregation, aggregationParam, formula, isAutoGenerated, hasUserReference, " \
              "fields{ id}}, ... on ColumnField{ dataType, dataCategory, role, defaultFormat, semanticRole, " \
...
DATASOURCE_QUERY = 'query PagedFieldsQuery($afterToken: String) {datasourcesConnection(first:1000, ' \
                   ...
                   'hasUserReference, hasExtracts, extractLastRefreshTime, extractLastUpdateTime, ' \
...
FLOW_QUERY = 'query PagedFieldsQuery($afterToken: String) {flowsConnection(first:1000, after: $afterToken){ ' \
             ...
             'owner { id, luid, name, email, domain, uri, username}, dataQualityWarning { id}}, ' \
             'pageInfo{ endCursor, hasNextPage}, totalCount}}'
...
def get_catalog_data(tableauserverclient, tableauserver):
    ...
    site_list = get_query_results(tableauserver, 'tableauSites', SITES_QUERY)

    print('{0}Found {1} Site(s) ...{2}'.format(bcolors.OKBLUE, len(site_list), bcolors.ENDC))
    ...
    # Pass the query onto the metadata api
    try:
        table_list = get_paged_query_results(tableauserver, 'tablesConnection', TABLE_QUERY)
    except IndexError as ie:
        print(ie)
        table_list = []
    ...
    # Pass the query onto the metadata api
    try:
        column_list = get_paged_query_results(tableauserver, 'columnsConnection', COLUMN_QUERY)
    except IndexError as ie:
        print(ie)
        column_list = []
    ...
    # Pass the query onto the metadata api
    try:
        virtual_connection_list = get_paged_query_results(tableauserver, 'virtualConnectionsConnection', VCONN_QUERY)
    except IndexError as ie:
        print(ie)
        virtual_connection_list = []

    print('{0}Found {1} Virtual Connection(s) ...{2}'.format(bcolors.OKBLUE, len(virtual_connection_list), bcolors.ENDC))
    ...
    # Pass the query onto the metadata api
    try:
        view_list = get_paged_query_results(tableauserver, 'viewsConnection', VIEW_QUERY)
    except IndexError as ie:
        print(ie)
        view_list = []
    ...
    # Pass the query onto the metadata api
    try:
        metric_list = get_paged_query_results(tableauserver, 'metricsConnection', METRIC_QUERY)
    except IndexError as ie:
        print(ie)
        metric_list = []
    ...
    # Pass the query onto the metadata api
    try:
        dqw_list = get_paged_query_results(tableauserver, 'dataQualityWarningsConnection', DATA_QUALITY_WARNING_QUERY)
    except IndexError as ie:
        print(ie)
        dqw_list = []
    ...
    try:
        dqc_list = get_paged_query_results(tableauserver, 'dataQualityCertificationsConnection',
                                           DATA_QUALITY_CERTIFICATION_QUERY)
    except IndexError as ie:
        print(ie)
        dqc_list = []
    ...
    # Pass the query onto the metadata api
    try:
        flow_list = get_paged_query_results(tableauserver, 'flowsConnection', FLOW_QUERY)
    except IndexError as ie:
        print(ie)
        flow_list = []
    ...
    # Pass the query onto the metadata api
    try:
        tag_list = get_paged_query_results(tableauserver, 'tagsConnection', TAG_QUERY)
    except IndexError as ie:
        print(ie)
        tag_list = []
    ...
    # Pass the query onto the metadata api
    try:
        lenses_list = get_paged_query_results(tableauserver, 'lensesConnection', LENS_QUERY)
    except IndexError as ie:
        print(ie)
        lenses_list = []
...
def get_query_results(tableauserver, table_type, query):
    df_results = []

    query_results = tableauserver.metadata.query(query)
...
def get_paged_query_results(tableauserver, table_type, query):
